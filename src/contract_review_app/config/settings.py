...
class Settings(BaseSettings):
    ...
    CONTRACT_REVIEW_MODEL_MAX_ATTEMPTS: int = 5
    CONTRACT_REVIEW_MODEL_RETRY_BACKOFF_SECONDS: float = 0.25
    CONTRACT_REVIEW_MODEL_RETRY_JITTER_RATIO: float = 0.2
    CONTRACT_REVIEW_MODEL_MAX_BACKOFF_SECONDS: float = 30.0
    # 连续临时失败时先 fail-fast，冷却后只允许一个 half-open 探针。
    CONTRACT_REVIEW_MODEL_CIRCUIT_BREAKER_ENABLED: bool = True
    CONTRACT_REVIEW_MODEL_CIRCUIT_FAILURE_THRESHOLD: int = 3
    CONTRACT_REVIEW_MODEL_CIRCUIT_OPEN_TIMEOUT_SECONDS: float = 30.0
    # 阶段 2 模型级进程内共享上限；默认保守为 1，避免尚未压测的配置直接放大流量。
    CONTRACT_REVIEW_MODEL_MAX_CONCURRENCY: int = 1
    CONTRACT_REVIEW_MODEL_QUEUE_TIMEOUT_SECONDS: float = 30.0
    # 阶段 4 自适应仅影响下一次审查；默认关闭，必须显式开启并保持 1～3 边界。
    CONTRACT_REVIEW_MODEL_ADAPTIVE_CONCURRENCY_ENABLED: bool = False
    CONTRACT_REVIEW_MODEL_ADAPTIVE_MIN_CONCURRENCY: int = 1
    CONTRACT_REVIEW_MODEL_ADAPTIVE_INITIAL_CONCURRENCY: int = 1
    CONTRACT_REVIEW_MODEL_ADAPTIVE_MAX_CONCURRENCY: int = 3
    CONTRACT_REVIEW_MODEL_ADAPTIVE_TARGET_LATENCY_SECONDS: float = 60.0
    ...
    CONTRACT_REVIEW_EMBEDDING_TIMEOUT_SECONDS: float = 120.0
    CONTRACT_REVIEW_EMBEDDING_MAX_CONCURRENCY: int = 1
