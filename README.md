...
版本号以 `pyproject.toml` 的 `[project].version` 为准，变更记录维护在 [CHANGELOG.md](CHANGELOG.md)。完整的版本、标签和 Release 操作步骤见 [发布流程](docs/release.md)；推送符合格式的 `vMAJOR.MINOR.PATCH` 标签后，GitHub Actions 会校验版本与变更日志，并创建同名 GitHub Release。
