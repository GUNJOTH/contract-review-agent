...
3. 每条适用规则先通过 `RetrievalQuery → RetrievalTrace → CandidateEvidence` 获取候选，再通过 `EvidenceAssessment` 裁决候选资格；关键词扫描只能参与候选生成。确定性事实抽取和规则检查器不得消费 `INSUFFICIENT`/`REJECT` 候选，语义模型不得访问候选之外的正文，候选为空或资格不足时缺失判断只能保持 `UNKNOWN`。
